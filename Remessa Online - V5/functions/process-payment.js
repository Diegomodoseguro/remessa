...
async function emitirCoris(leadData) {
    ...
    const gravarParams = {
        ...
        'telefone': (leadData.comprador.telefone || leadData.contactPhone || '00000000000').replace(/\D/g, ''),
        'pagamento': 'CARTAO' 
    };
...
exports.handler = async (event) => {
    ...
    const { paymentMethodId, leadId: idDoLead, planId, amountBRL, comprador, passageiros, planName, dates, destination, contactPhone } = body;
    leadId = idDoLead;
    ...
    try {
        ...
        try {
            corisData = await emitirCoris({ leadId, planId, destination, passengers: passageiros, comprador, contactPhone: contactPhone, dates });
        } catch (corisError) {
        ...
        // Finalização: Atualiza Supabase com TODOS os dados
        await supabaseClient.from('remessaonlinesioux_leads')
            .update({
                ...
                valor_final_brl: amountBRL, // CORREÇÃO IMPORTANTE
                plano_escolhido: planName, // CORREÇÃO IMPORTANTE
                passageiros_info: JSON.stringify(passageiros), // CORREÇÃO IMPORTANTE: Salva JSON com nomes/CPFs
